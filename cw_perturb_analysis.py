...
def mnist_tutorial_cw(train_start=0, train_end=60000, test_start=0,
  ...
  num_adv_samples = len(adv_inputs)
  ...
  for i in range(num_perturb_samples):
    ...
    binary_preds = binary_model.get_logits(x)
    binary_probs = tf.nn.softmax(binary_preds)[:, 0]
    ...
    original = sess.run(binary_probs, feed_dict = {x: adv_inputs})
    perturbed = sess.run(binary_probs, feed_dict = {x: adv})
    print('Average magnitude score change: ' + str(np.mean(np.abs(perturbed - original))))
    D[:, i] = (perturbed - original)
  np.savetxt("cw_perturbations.csv", D, delimiter=",")
